// MAXIS STRIKE Server v7.0
// NO ANTICHEAT, Server cheats, custom maps JSON, dynamic arena
...
// Custom maps stored in JSON file
const CUSTOM_MAPS_FILE = path.join(__dirname, 'custom_maps.json');
let customMaps = {};
try { customMaps = JSON.parse(fs.readFileSync(CUSTOM_MAPS_FILE, 'utf8')); } catch(e) { customMaps = {}; }
function saveCustomMaps() {
  try { fs.writeFileSync(CUSTOM_MAPS_FILE, JSON.stringify(customMaps, null, 2)); } catch(e) {}
}
...
// ═══════════════════════════════════════════
// WEBSOCKET — NO ANTICHEAT
// ═══════════════════════════════════════════
const wss = new WebSocket.Server({ server });

console.log('═══════════════════════════════════════');
console.log('  MAXIS STRIKE Server v7.0');
console.log('  Port:', PORT);
console.log('  NO ANTICHEAT — Stable gameplay');
console.log('  Custom maps saved to JSON');
console.log('═══════════════════════════════════════');
...
function handleMessage(ws, client, data) {
  switch(data.type) {
    ...
    case 'getCustomMapData': sendCustomMapData(ws, data.mapName); break;
    case 'searchMaps': searchMaps(ws, data.query); break;
  }
}

// ═══════════════════════════════════════════
// CUSTOM MAPS — saved to JSON file
// ═══════════════════════════════════════════
function handlePublishMap(ws, client, data) {
  ...
  broadcastAll({ type: 'customMapsUpdate', maps: Object.keys(customMaps) });
  console.log(`Custom map published: ${mapName}`);
}
...
function searchMaps(ws, query) {
  const q = (query || '').toLowerCase();
  const results = Object.keys(customMaps).filter(n => n.toLowerCase().includes(q));
  ws.send(JSON.stringify({ type: 'mapSearchResults', maps: results }));
}
...
function createRoom(ws, client, data) {
  ...
  const room = {
    ...
    mode: data.mode || 'deathmatch', maxPlayers: mapCfg.maxPlayers || 16,
  ...
  if (mapName === 'MS_ARENA_BETA') {
    ...
    room.physicsObjects = generatePhysics(room.mapSize, 1);
  } else {
...
function generateMapPhysics(mapName) {
  const objects = [];
  let id = 1;
  const positions = mapName === 'MS_DUST' ?
    [{ x: -20, z: -15 }, { x: 20, z: 15 }, { x: -50, z: 0 }, { x: 50, z: 0 }, { x: 0, z: -30 }, { x: 0, z: 30 }] :
    [{ x: -15, z: 0 }, { x: 15, z: 0 }, { x: 0, z: -20 }, { x: 0, z: 20 }, { x: -30, z: -30 }, { x: 30, z: 30 }];
...
function generatePhysics(size, playerCount) {
  ...
  for (let i = 0; i < playerCount; i++) {
    ...
    barrels.push({ x: Math.cos(angle) * dist, z: Math.sin(angle) * dist });
  }
  ...
  for (let i = 0; i < Math.floor(playerCount / 2); i++) {
    ...
    crates.push({ x: Math.cos(angle) * dist, z: Math.sin(angle) * dist });
  }
...
function joinRoom(ws, client, data) {
  ...
  if (room.map === 'MS_ARENA_BETA') {
    ...
    broadcastToRoom(room.id, {
      type: 'mapResize', size: room.mapSize,
...
function handleMove(ws, client, data) {
  ...
  // Push physics objects when walking near them
...
function handleShoot(ws, client, data) {
  ...
  // Push physics objects with bullets
  if (weapon !== 'rpg' && weapon !== 'knife') {
    for (const obj of room.physicsObjects) {
      if (obj.destroyed) continue;
      const hitDist = checkRayHitSphere(
        data.x, data.y, data.z, data.dx, data.dy, data.dz,
        obj.x, obj.y, obj.z, obj.type === 'barrel' ? obj.radius : obj.size / 2
      );
  ...
  if (weapon === 'rpg') return; // RPG handled by rpgExplode
  ...
  for (let pid in room.players) {
    ...
    const dist = checkRayHitSphere(data.x, data.y, data.z, data.dx, data.dy, data.dz, target.x, target.y, target.z, 0.8);
    if (dist !== null && dist < 100) {
      ...
      broadcastToRoom(room.id, {
        type: 'bloodEffect',
        x: target.x, y: target.y, z: target.z, headshot
      });
...
function handleKill(room, client, shooter, target, targetId, weapon, headshot) {
  ...
  if (!room.firstBloodTaken) {
    ...
    grantAchievement(client, 'FIRST_BLOOD');
  }
  if (weapon === 'knife') grantAchievement(client, 'KNIFE_MASTER');
  if (client.stats.headshots >= 10) grantAchievement(client, 'HEADHUNTER');
  if (shooter.killStreak >= 5) grantAchievement(client, 'RAMPAGE');
...
function handleRPGExplosion(ws, client, data) {
  ...
  for (let pid in room.players) {
    ...
    if (dist < explosionRadius) {
      ...
      const damage = Math.floor(150 * falloff);
      target.hp -= damage;
...
function handleDestroyWall(ws, client, data) {
  ...
  if (wall.hp <= 0) {
    ...
    broadcastToRoom(room.id, { type: 'wallDestroyed', wallId: wall.id });
    if (client.stats.wallsDestroyed >= 10) grantAchievement(client, 'DEMOLITION');
  } else {
...
function handleBuyWeapon(ws, client, data) {
  ...
  if (player.money >= 10000) grantAchievement(client, 'RICH');
}
...
function handleWeaponSwitch(ws, client, data) {
  ...
  if (!room || !room.players[client.id]) return;
  const player = room.players[client.id];
  if (!player.weapons.includes(data.weapon)) return;
...
// ═══════════════════════════════════════════
// SERVER CHEAT — Alt+W sends to server, server adds money
// ═══════════════════════════════════════════
function handleCheat(ws, client, data) {
  ...
  if (data.code === 'money5000') {
    ...
    console.log(`CHEAT: Player ${client.id} +5000 money (now ${player.money})`);
    if (player.money >= 10000) grantAchievement(client, 'RICH');
  }
}
...
function grantAchievement(client, achId) {
  if (client.achievements.includes(achId)) return;
  client.achievements.push(achId);
  if (achievements[achId]) {
    sendToPlayerWS(client.id, { type: 'newAchievements', achievements: [achievements[achId]] });
  }
}
...
// UTILS
// ═══════════════════════════════════════════
function checkRayHitSphere(ox, oy, oz, dx, dy, dz, tx, ty, tz, radius) {
...
setInterval(() => {
  for (const roomId in rooms) {
    ...
    if (Object.keys(room.players).length > 0) {
      ...
      if (room.hostages && room.hostages.length > 0) stateData.hostages = room.hostages;
